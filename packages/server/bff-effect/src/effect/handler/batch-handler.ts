...
import {
  ...
  isBatchBody,
  toBatchResponseItem,
} from '../../data-platform/batch/protocol';
import {
...
async function readRequestTextWithinLimit(
  ...
  if (contentLength !== null) {
    if (!/^\d+$/.test(contentLength)) {
      return { status: 'invalid-content-length' };
    }
  ...
  try {
    while (true) {
      ...
      if (chunk.done) {
        ...
        return { status: 'ok', text: text.join('') };
      }
      bytesRead += chunk.value.byteLength;
      if (bytesRead > maxBytes) {
        await reader.cancel('batch-payload-too-large');
        return { status: 'too-large' };
      }
...
export function createDataPlatformBatchRequestHandler<TContext>(options: {
  ...
  const handle = async (request: Request, context?: TContext) => {
    ...
    const bodyResult = await readRequestTextWithinLimit(request, batchMaxBytes);
    ...
    if (bodyResult.status === 'invalid-content-length') {
      return createBatchValidationResponse(
        'Batch payload has invalid content-length',
      );
    }
    if (bodyResult.status === 'read-error') {
      return createBatchValidationResponse('Unable to read batch payload');
    }
    const payloadText = bodyResult.text;
    ...
    const responseItems = await mapWithConcurrency(
      ...
      async (rawItem, index) => {
        ...
        if (typeof rawItem.body !== 'undefined' && !isBatchBody(rawItem.body)) {
          return toBatchItemError(
            itemId,
            400,
            'Batch item body must use the base64 body protocol',
          );
        }
