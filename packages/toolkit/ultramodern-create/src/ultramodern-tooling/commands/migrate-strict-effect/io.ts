...
function resolvePotentialRealPath(
  ...
  try {
    ...
    if (visitedSymlinks.has(existingAncestor)) {
      throw new Error(
        `Refusing to follow cyclic migration symlink: ${existingAncestor}`,
      );
    }
    visitedSymlinks.add(existingAncestor);
    realAncestor = resolvePotentialRealPath(
      path.resolve(
        path.dirname(existingAncestor),
        fs.readlinkSync(existingAncestor),
      ),
      visitedSymlinks,
    );
  }
...
function copyWorkspaceEntry(
  ...
  sourceWorkspaceAliases: readonly string[],
) {
...
function relocateWorkspacePath(
  ...
  for (const sourceWorkspaceRoot of sourceWorkspaceRoots) {
    const relativePath = path.relative(sourceWorkspaceRoot, sourcePath);
    if (isPathInsideRoot(sourceWorkspaceRoot, sourcePath)) {
...
function createMigrationIoWithBehavior(
  ...
  const realWorkspaceRoot = resolvePotentialRealPath(absoluteWorkspaceRoot);
  const plan: string[] = [];
  ...
  const assertRealWorkspacePath = (
    absolutePath: string,
    displayedPath: string,
    followFinalSymlink = true,
  ) => {
    const pathToResolve = followFinalSymlink
      ? absolutePath
      : path.dirname(absolutePath);
    const realPath = resolvePotentialRealPath(pathToResolve);
    if (!isPathInsideRoot(realWorkspaceRoot, realPath)) {
      throw new Error(`Refusing to mutate outside workspace: ${displayedPath}`);
    }
  };

  const assertWorkspacePath = (filePath: string, followFinalSymlink = true) => {
    const absolutePath = path.resolve(filePath);
    ...
    assertRealWorkspacePath(absolutePath, filePath, followFinalSymlink);
    return absolutePath;
  };
  ...
  const io: MigrationIo = {
    ...
    write(filePath, content) {
      ...
      if (current?.isSymbolicLink()) {
        ...
        assertRealWorkspacePath(absoluteTargetPath, filePath);
        captureMissingParents(absoluteTargetPath);
        captureForRollback(absoluteTargetPath);
      }
      assertRealWorkspacePath(absolutePath, filePath);
    ...
    remove(filePath) {
      const absolutePath = assertWorkspacePath(filePath, false);
      ...
      captureForRollback(absolutePath);
      assertRealWorkspacePath(absolutePath, filePath, false);
    ...
    withStagedWorkspace(operation) {
      ...
      const sourceWorkspaceAliases = [
        ...new Set([absoluteWorkspaceRoot, realWorkspaceRoot]),
      ];
      try {
        copyWorkspaceEntry(
          realWorkspaceRoot,
