import type {
  HttpMethodDecider,
  IOptions as NativeOptions,
  RequestCreatorOptions as NativeRequestCreatorOptions,
  UploadCreatorOptions as NativeUploadCreatorOptions,
  Sender,
...
export type RequestCreatorOptions<F = typeof fetch> = Omit<
  NativeRequestCreatorOptions<F>,
  'hooks'
...
export type UploadCreatorOptions = Omit<NativeUploadCreatorOptions, 'hooks'> & {
  operationContext?: OperationContext;
};

export type UploadCreator = (options: UploadCreatorOptions) => Sender;

export type IOptions<F = typeof fetch> = NativeOptions<F> & {
