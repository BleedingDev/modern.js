...
describe('Node deployment npm aliases', () => {
  it('preserves emitted aliases through the native deployment completion hook', async () => {
    const appDirectory = await mkdtemp(
      path.join(tmpdir(), 'deploy-alias-hook-'),
    );
    ...
    try {
      await writeJson(path.join(appDirectory, 'package.json'), {
        name: 'alias-hook-app',
        dependencies: { '@app/core': 'npm:@vendor/core@1.0.0' },
      });
      await writeJson(path.join(outputDirectory, 'package.json'), {
        name: 'alias-hook-output',
      });
      await writeJson(
        path.join(outputDirectory, 'node_modules/@vendor/core/package.json'),
        { name: '@vendor/core', version: '1.0.0', main: 'index.js' },
      );
      await writeFile(
        path.join(outputDirectory, 'node_modules/@vendor/core/index.js'),
        'module.exports = "hook-runtime";',
      );
      const identity = await readPackageIdentity(
        createRequire(__filename).resolve('@modern-js/prod-server'),
      );
      await writeJson(
        path.join(
          outputDirectory,
      ...
      await callbacks[0]();
      expect(
        createRequire(path.join(outputDirectory, 'index.js'))('@app/core'),
      ).toBe('hook-runtime');
      const manifest = JSON.parse(
        await readFile(path.join(outputDirectory, 'package.json'), 'utf8'),
      );
      expect(manifest.dependencies['@app/core']).toBe('npm:@vendor/core@1.0.0');
    } finally {
