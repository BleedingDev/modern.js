---
name: um-zero-20260909-localization
overview: "Repair localization ownership and native locale integration. packages/runtime/plugin-i18n and packages/toolkit/i18n-utils, excluding shared package manifests and changelogs."
todos:
  - id: zero-localization-extract
    content: "Repair every assigned inventory row by moving genuine fork additions into i18n-extensions and established runtime plugin hooks, deleting their old implementations and redundant helpers, and retaining audited identities under their approved legal route."
    status: pending
  - id: zero-localization-integrate
...
# Repair localization ownership and native locale integration

## Execution Notes

Tracking: `modernjs-cdhz.9`. Parent planning program: `modernjs-cdhz`. **Planning only: every implementation todo remains pending.** No subagents are launched by this plan. Read [the program contract](um-zero-20260909-overview.md) and [initial ownership coverage](notes/um-zero-20260909/ownership-coverage.md).

Remove all six current i18n edges while preserving URL language negotiation, redirects, cookie/header behavior, external i18next stores, Suspense safety and client/server identity. Keep native plugin APIs; no custom application navigation or per-app locale patches. Initial reservations are the exact rows in debt-inventory.json; package manifests belong to packaging and changelogs to docs-ownership. Any shared-file change requires an explicit handoff. Code may be developed against an exact prepared upstream proposal, but cannot be integrated or released as if that proposal had already merged.

## Constraints

Own: packages/runtime/plugin-i18n and packages/toolkit/i18n-utils, excluding shared package manifests and changelogs.
