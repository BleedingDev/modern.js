---
name: um-parallel-20260909-audit-server-request-contracts
...
todos:
  - id: audit-server-request-contracts-reconcile
    ...
    status: pending
  - id: audit-server-request-contracts-route
    ...
    status: pending
  - id: audit-server-request-contracts-grant
...
# Audit: Request contracts, server actions and safe-failure ownership

## Execution Notes

Tracking: `modernjs-cdhz.29.31`. Mode: **read-only**. Read [the parallel program](um-parallel-20260909-overview.md) and [the unchanged acceptance contract](um-zero-20260909-overview.md). This is a deferred execution plan; creating it does not perform its tasks.
...
Required output: Per-identity legal route, caller/seam evidence, complete-PR capacity and one exact source/destination ownership packet for server-request-contracts.
...
Owner: `audit-server-request-contracts`. Completion updates this plan and its exact br issue only after evidence is reviewed. Never mark a task complete because it is queued, drafted, or blocked.
