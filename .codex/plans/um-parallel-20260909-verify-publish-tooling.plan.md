---
name: um-parallel-20260909-verify-publish-tooling
overview: "Run required publication-tooling and source/packed/receipt ownership tests; no prior-source receipt reuse."
todos:
  - id: verify-publish-tooling-deliver
    content: "Run required publication-tooling and source/packed/receipt ownership tests; no prior-source receipt reuse."
    status: pending
  - id: verify-publish-tooling-prove
    content: "Produce and verify: Exact-ref check receipts with toolchain, workspace and artifact identities."
    status: pending
isProject: false
---

# Verify publication and acceptance tooling

## Execution Notes

Tracking: `modernjs-cdhz.29.112`. Mode: **verification-only**. Read [the parallel program](um-parallel-20260909-overview.md) and [the unchanged acceptance contract](um-zero-20260909-overview.md). This is a deferred execution plan; creating it does not perform its tasks.

Run required publication-tooling and source/packed/receipt ownership tests; no prior-source receipt reuse.
...
Owner: `verify-publish-tooling`. Completion updates this plan and its exact br issue only after evidence is reviewed. Never mark a task complete because it is queued, drafted, or blocked.
...
Resource locks: `private-checkout:verify-publish-tooling`, `private-build-root:verify-publish-tooling`

Verification: Run required publication-tooling and source/packed/receipt ownership tests; no prior-source receipt reuse.
