...
test('finds duplicate stylesheet hrefs in each browser validation phase', async () => {
  const { findDuplicateStylesheetHrefs } = await loadSmoke();
  const duplicateHref = 'https://shell.example/static/app.css';

  for (const [phase, stylesheetHrefs] of [
    [
      'pre-federation-hydration',
      [
        duplicateHref,
    ...
    ['no-javascript', [duplicateHref, duplicateHref]],
    [
      'hydrated',
      [
        duplicateHref,
    ...
    assert.deepEqual(
      findDuplicateStylesheetHrefs(stylesheetHrefs),
