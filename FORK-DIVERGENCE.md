...
| `packages/runtime/plugin-i18n/tests/linkTypes.test.ts` | bleedingdev | Inspect the required built declarations without rebuilding shared package output during test collection, preventing concurrent native CLI imports from observing a partially rewritten dist tree. Preserve the strict declaration and dependency assertions. | inline-patch |
